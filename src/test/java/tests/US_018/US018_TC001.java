...
public class US018_TC001 {
    ...
    List<String> list;
    @Test public void US018_TC018() throws InterruptedException {

      //  Vendor https://pearlymarket.com/ sitesine gider
     ...
     //   jse.executeScript("arguments[0].click();", c4PearlyMarketPage.);
        ...
        //Vendor (*) talep nedenini belirtir ve submit i tıklar
        c4PearlyMarketPage.partialRefund.sendKeys(Keys.ENTER);
        actions.sendKeys(Keys.ARROW_UP).sendKeys(Keys.ENTER).sendKeys(Keys.TAB).sendKeys("Bozuk").perform();
        c4PearlyMarketPage.submitRequest.click();
        Thread.sleep(4000);
        //Vendor Refund a gider ve request detaylarını görüntüler
        c4PearlyMarketPage.refundButton.click();
        list=new ArrayList<>();
        for (WebElement w: c4PearlyMarketPage.refunList) {
           list.add(w.getText()) ;
        }
        list.forEach(System.out::println);
        Assert.assertFalse(c4PearlyMarketPage.refunList.isEmpty());
    }
